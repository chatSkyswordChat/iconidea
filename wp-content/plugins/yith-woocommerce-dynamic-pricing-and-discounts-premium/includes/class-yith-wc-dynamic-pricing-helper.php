	...
	class YITH_WC_Dynamic_Pricing_Helper {
		...
		public function validate_apply_to( $key_rule, $rule, $cart_item_key, $cart_item ) {
			...
			if ( $is_valid ) {
				...
				if ( ! $is_active_apply_adjustment || ( $is_active_apply_adjustment && 'all_products' === $rule['rule_apply_adjustment_discount_for'] ) || $is_valid_also_for_adjust ) {
			...
			if ( ! $is_in_exclusion ) {
				...
				switch ( $rule['rule_for'] ) {
					...
					case 'vendor_list':
						...
						break;
					default:
						$is_valid = apply_filters( 'ywdpd_validate_apply_to', $is_valid, $rule['rule_for'], $cart_item['product_id'], $rule, $cart_item );
		...
		public function valid_product_to_apply( $rule, $product, $other_variations = false ) {
			...
			$this->valid_product_to_apply[ $key_check ] = apply_filters( 'ywdpd_is_valid_product_to_apply', $is_valid, $rule, $product, $other_variations );
		...
		public function valid_num_of_orders_cart_condition( $condition, $conditions ) {
			...
			if ( is_user_logged_in() ) {
				...
				$min_order    = ! empty( $condition['min_order'] ) ? $condition['min_order'] : 1;
		 ...
		 * @return float
		 */
		public function get_cart_subtotal( $conditions ) {
			$subtotal = 0;
