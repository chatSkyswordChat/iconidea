...
if ( ! class_exists( 'YWDPD_Brands' ) ) {
	...
	class YWDPD_Brands {
		...
		public function __construct() {
			...
			add_filter( 'ywdpd_is_valid_product_to_apply', array( $this, 'is_valid_brand_to_apply' ), 20, 3 );
			add_filter( 'ywdpd_is_valid_product_apply_adjustment_to', array(
				$this,
				'is_valid_brand_apply_adjustment_to'
			), 20, 3 );

			add_filter( 'ywdpd_validate_apply_to', array( $this, 'is_validate_apply_to_cart_item' ), 10, 5 );
		 ...
		 * @return bool
		 */
		public function is_valid_brand_to_apply( $is_valid, $rule, $product ) {
		 ...
		 * @return bool
		 */
		public function is_validate_apply_to_cart_item( $is_valid, $apply_to, $product_id, $rule, $cart_item ) {

			$product  = wc_get_product( $product_id );
			$is_valid = $this->is_valid_brand_to_apply( $is_valid, $rule, $product );
