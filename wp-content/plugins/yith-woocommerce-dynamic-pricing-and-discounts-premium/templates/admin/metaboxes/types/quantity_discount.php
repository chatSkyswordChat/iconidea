...
<div id="<?php echo esc_attr( $id ); ?>-container" <?php echo yith_field_deps_data( $args ); //phpcs:ignore ?>
		...
		<div class="discount-table-rules-wrapper">
			<div class="discount-rules">
				...
				for ( $i = 1; $i <= $limit; $i ++ ) :
					...
					*/
						echo sprintf( esc_html__( '- From %1$s to %2$s apply %3$s %4$s %5$s', 'ywdpd' ), $min_field, $max_field, $discount_type_field, $amount_fields, $symbol_field );
						if ( 1 !== $i ):
							?>
			...
			<div class="discount_new_rule">
				<a href="#" id="ywdpd_new_qty_rule" class="ywdpd_new_rule"><?php _e( '+ Add rule', 'ywdpd' ); ?></a>
			</div>
		</div>
	...
	<div class="clear"></div>
	<span
		class="description"><?php _e( 'Create the quantity rules for the product you selected. Leave "to" field empty, to not put a max amount in this rule', 'ywdpd' ); ?></span>
</div>
