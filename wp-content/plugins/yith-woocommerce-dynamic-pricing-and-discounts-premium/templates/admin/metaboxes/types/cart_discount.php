	...
	<div class="yith-plugin-fw-field-wrapper">
		<div id="cart-rules">
			...
			for ( $i = 1; $i <= $limit; $i ++ ):
				...
				<div class="cart-rule-row" data-index="<?php esc_attr_e( $i ); ?>">
					<div class="cart-rule-single-field">
						<div id="cart-rule-single-rule-type-container-<?php esc_attr_e( $i ); ?>" >

							<label for="rules_type_<?php esc_attr_e( $i ); ?>"><?php esc_html_e( 'Set a conditions based on', 'ywdpd' ); ?></label>
							<?php
								echo yith_plugin_fw_get_field( $rule_type_field ); //phpcs:ignore
							?>
						</div>
						<div id="cart-rule-single-rule-sub-type-container-<?php esc_attr_e( $i ); ?>" >
							<?php
							foreach ( $cart_rules_options['rules_type'] as $cart_rule_key => $cart_rule_option ):
								...
								?>
								<div class="cart-rule-single-rule-sub-select <?php echo $hide_class; ?>"
									 data-type="<?php esc_attr_e( $cart_rule_key ); ?>">
									<label
										for="rule_type_<?php echo $cart_rule_key; ?>_<?php echo $i; ?>"><?php esc_html_e( 'for', 'ywdpd' ); ?></label>
									<?php
									...
									echo yith_plugin_fw_get_field( $field_args );
									?>
								</div>
							<?php
							endforeach;
						...
						<div id="cart-rule-single-rule-sub-sub-type-container-<?php esc_attr_e( $i);?>" class="ywdpdp_inline">
							<?php
							foreach ( $cart_rules_options['rules_type'] as $cart_rule_key => $cart_rule_option ):
								$hide_class = $rule_type_value !== $cart_rule_key ? 'ywdpd_hide_row' : '';
								$options_key = array_keys( $cart_rule_option['options'] );
		...
		<div class="cart_new_rule">
			<a href="#" id="ywdpd_new_cart_rule" class="ywdpd_new_rule"><?php _e( '+ Add condition', 'ywdpd' ); ?></a>
		</div>
		<div class="clear"></div>
			...
			<div id="cart-rule-single-rule-sub-type-container-{{{data.index}}}" >
				...
				foreach ( $cart_rules_options['rules_type'] as $cart_rule_key => $cart_rule_option ):
					...
					<div class="cart-rule-single-rule-sub-select <?php echo $hide_class; ?>"
						 data-type="<?php esc_attr_e( $cart_rule_key ); ?>">
						<label
							for="rule_type_<?php echo $cart_rule_key; ?>_{{{data.index}}}"><?php esc_html_e( 'for', 'ywdpd' ); ?></label>
						<?php
