...
class wp_line_notify{
    ...
    public function html_settings_page() {
        ...
        $web_users = wp_line_notify::sig_get_all_users();
     ...
     *  return string
     */
    public static function current_user_option_name(){

        $option_name = SIG_LINE_NOTIFY_OPTIONS;

        if ( isset($_GET['uid']) && preg_match("/^[1-9][0-9]*$/" ,$_GET['uid']) ) {
             $option_name .= '_'.$_GET['uid'];
        }
        return $option_name;
    }
    ...
    public function sig_line_notify_test(){
        ...
        $uid = $_POST['line_notify_uid_test'];
        $option_name = SIG_LINE_NOTIFY_OPTIONS;
        if( !empty($uid) && preg_match("/^[1-9][0-9]*$/" ,$uid) ) {
            $option_name .= '_'.$uid;
        }
